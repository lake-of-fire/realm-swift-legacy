...
/**
 ...
 - see: `LEGACYUser`
 */
public typealias User = LEGACYUser
...
/**
 ...
 - see: `LEGACYSyncManager`
 */
public typealias SyncManager = LEGACYSyncManager

/**
 Options for configuring timeouts and intervals in the sync client.

  - see: `LEGACYSyncTimeoutOptions`
 */
public typealias SyncTimeoutOptions = LEGACYSyncTimeoutOptions
...
/**
 ...
 - see: `LEGACYSyncSession`
 */
public typealias SyncSession = LEGACYSyncSession

/**
 A closure type for a closure which can be set on the `SyncManager` to allow errors to be reported
 to the application.

 - see: `LEGACYSyncErrorReportingBlock`
 */
public typealias ErrorReportingBlock = LEGACYSyncErrorReportingBlock

/**
 A closure type for a closure which is used by certain APIs to asynchronously return a `SyncUser`
 object to the application.

 - see: `LEGACYUserCompletionBlock`
 */
public typealias UserCompletionBlock = LEGACYUserCompletionBlock
...
/**
 ...
 - see: `LEGACYSyncError`
 */
public typealias SyncError = LEGACYSyncError
...
extension SyncError {
    /**
     ...
     - see: `LEGACYSyncErrorActionToken`
     */
    public typealias ActionToken = LEGACYSyncErrorActionToken
    ...
    public func clientResetInfo() -> (String, SyncError.ActionToken)? {
        if code == SyncError.clientResetError,
            let recoveryPath = userInfo[kLEGACYSyncPathOfRealmBackupCopyKey] as? String,
    ...
    public var serverLogURL: URL? {
        (userInfo[LEGACYServerLogURLKey] as? String).flatMap(URL.init)
    }
    ...
    public var compensatingWriteInfo: [CompensatingWriteInfo]? {
        userInfo[LEGACYCompensatingWriteInfoKey] as? [CompensatingWriteInfo]
    }
}
...
/// without warning.
public typealias CompensatingWriteInfo = LEGACYCompensatingWriteInfo
 ...
 */
public typealias AppError = LEGACYAppError

extension AppError {
    /// When applicable, the HTTP status code which resulted in this error.
    var httpStatusCode: Int? {
        userInfo[LEGACYHTTPStatusCodeKey] as? Int
    }
}
...
/**
 ...
 - see: `LEGACYSyncLogLevel`
 */
public typealias SyncLogLevel = LEGACYSyncLogLevel

/**
 A data type whose values represent different authentication providers that can be used with
 Atlas App Services.

 - see: `LEGACYIdentityProvider`
 */
public typealias Provider = LEGACYIdentityProvider
...
@frozen public enum ClientResetMode {
    ...
    /// - seeAlso ``LEGACYClientResetBeforeBlock``
    ...
    /// - seeAlso ``LEGACYClientResetAfterBlock``
    ///
    /// Example Usage
    /// ```
    ...
    case recoverOrDiscardUnsyncedChanges(beforeReset: (@Sendable (_ before: RealmLegacy) -> Void)? = nil,
                                         afterReset: (@Sendable (_ before: RealmLegacy, _ after: RealmLegacy) -> Void)? = nil)
    /// - seeAlso: ``LEGACYClientResetModeManual``
    ///
    ...
    /// - seeAlso: ``LEGACYSyncManager.errorHandler``
    ///
    /// During an ``LEGACYSyncErrorClientResetError`` the block executed is determined by the following rules
    ...
    /// - note: The ``SyncManager.errorHandler`` is still invoked under all ``LEGACYSyncError``s *other than* ``LEGACYSyncErrorClientResetError``.
    /// - seeAlso ``LEGACYSyncError`` for an exhaustive list.
    @preconcurrency
...
@frozen public struct SyncConfiguration: Sendable {
    ...
    /**
     ...
     - see: ``ClientResetMode`` and ``LEGACYClientResetMode``
     - see: https://docs.mongodb.com/realm/sync/error-handling/client-resets/
    */
    ...
    @Unchecked internal var config: LEGACYSyncConfiguration
    internal init(config: LEGACYSyncConfiguration) {
        self.config = config
    }
}
...
@frozen public struct Functions: Sendable {
    ...
    public subscript(dynamicMember string: String) -> Function {
        return { (arguments: [AnyBSON], completionHandler: @escaping FunctionCompletionHandler) in
            ...
            self.user.__callFunctionNamed(string, arguments: objcArgs) { (bson: LEGACYBSON?, error: Error?) in
...
public struct FunctionCallable: Sendable {
    ...
    public func dynamicallyCall(withArguments args: [[AnyBSON]]) -> Future<AnyBSON, Error> {
        return future { promise in
            ...
            self.user.__callFunctionNamed(name, arguments: objcArgs) { (bson: LEGACYBSON?, error: Error?) in
                if let b = bson.map(ObjectiveCSupport.convertBson), let bson = b {
...
public extension User {
    ...
    func configuration(partitionValue: AnyBSON,
                       clientResetMode: ClientResetMode = .recoverUnsyncedChanges(beforeReset: nil, afterReset: nil),
                       cancelAsyncOpenOnNonFatalErrors: Bool = false) -> RealmLegacy.Configuration {
        var config: LEGACYRealmConfiguration
    ...
    */
    var customData: Document {
        guard let rlmCustomData = self.__customData as LEGACYBSON?,
...
public extension SyncSession {
    /**
     ...
     - see: `LEGACYSyncSessionState`
     */
    typealias State = LEGACYSyncSessionState

    /**
     The current state of a sync session's connection.

     - see: `LEGACYSyncConnectionState`
     */
    typealias ConnectionState = LEGACYSyncConnectionState
     ...
     */
    typealias ProgressNotificationToken = LEGACYProgressNotificationToken
...
@frozen public struct UserSubscription: Subscription {
    private let token: LEGACYUserSubscriptionToken

    internal init(token: LEGACYUserSubscriptionToken) {
        self.token = token
    }
