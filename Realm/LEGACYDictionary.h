...
@class LEGACYObject, LEGACYResults<LEGACYObjectType>, LEGACYDictionaryChange;

/**
 `LEGACYDictionary` is a container type in Realm representing a dynamic collection of key-value pairs.

 Unlike `NSDictionary`, `LEGACYDictionary`s hold a single key and value type.
 This is referred to in these docs as the “type” and “keyType” of the dictionary.

 When declaring an `LEGACYDictionary` property, the object type and keyType must be marked as conforming to a
 protocol by the same name as the objects it should contain.

     LEGACY_COLLECTION_TYPE(ObjectType)
     ...
     @property LEGACYDictionary<NSString *, ObjectType *><LEGACYString, ObjectType> *objectTypeDictionary;

 `LEGACYDictionary`s can be queried with the same predicates as `LEGACYObject` and `LEGACYResult`s.

 `LEGACYDictionary`s cannot be created directly. `LEGACYDictionary` properties on `LEGACYObject`s are
 lazily created when accessed, or can be obtained by querying a Realm.

 `LEGACYDictionary` only supports `NSString` as a key.  Realm disallows the use of `.` or `$` characters within a dictionary key.

 ### Key-Value Observing

 `LEGACYDictionary` supports dictionary key-value observing on `LEGACYDictionary` properties on `LEGACYObject`
 subclasses, and the `invalidated` property on `LEGACYDictionary` instances themselves is
 key-value observing compliant when the `LEGACYDictionary` is attached to a managed
 `LEGACYObject` (`LEGACYDictionary`s on unmanaged `LEGACYObject`s will never become invalidated).
 */
@interface LEGACYDictionary<LEGACYKeyType, LEGACYObjectType>: NSObject<LEGACYCollection>
 ...
 */
@property (nonatomic, readonly, assign) LEGACYPropertyType keyType;
 ...
 */
- (nullable id)valueForKey:(nonnull LEGACYKeyType)key;
 ...
 */
@property(readonly, copy) NSArray<LEGACYKeyType> *allKeys;
...
/**
 ...
 @note The order of the elements in the array is not defined.
 */
@property(readonly, copy) NSArray<LEGACYObjectType> *allValues;
 ...
 */
- (nullable LEGACYObjectType)objectForKey:(nonnull LEGACYKeyType)key;
...
/**
 ...
 @return The value associated with key.
 */
- (nullable LEGACYObjectType)objectForKeyedSubscript:(LEGACYKeyType)key;
 ...
 */
- (void)enumerateKeysAndObjectsUsingBlock:(void (^)(LEGACYKeyType key, LEGACYObjectType obj, BOOL *stop))block;

#pragma mark - Adding, Removing, and Replacing Objects in a Dictionary

/**
 Replace the contents of a dictionary with the contents of another dictionary - NSDictionary or LEGACYDictionary.
...
/**
 ...
 @warning This method may only be called during a write transaction.
 */
- (void)removeObjectsForKeys:(NSArray<LEGACYKeyType> *)keyArray;
...
/**
 ...
 @warning This method may only be called during a write transaction.
 */
- (void)removeObjectForKey:(LEGACYKeyType)key;
...
/**
 ...
 @warning This method may only be called during a write transaction.
 */
- (void)setObject:(nullable LEGACYObjectType)obj forKeyedSubscript:(LEGACYKeyType)key;
...
/**
 ...
 @warning This method may only be called during a write transaction.
 */
- (void)setObject:(nullable LEGACYObjectType)anObject forKey:(LEGACYKeyType)aKey;
...
/**
  ...
  @param otherDictionary An enumerable object such as `NSDictionary` or `LEGACYDictionary` which contains objects of the
...
/**
 ...
 @note The keys in the dictionary are ignored when quering values, and they will not be returned in the `LEGACYResults`.
...
/**
 Returns a sorted LEGACYResults of all values in the dictionary.

 @note The keys in the dictionary are ignored when sorting values, and they will not be returned in the `LEGACYResults`.
...
/**
 Returns a distinct `LEGACYResults` from all values in the dictionary.

 @note The keys in the dictionary are ignored, and they will not be returned in the `LEGACYResults`.
...
/**
 ...
 @param property The property whose minimum value is desired. Only properties of
                 types `int`, `float`, `double`, `NSDate`, `LEGACYValue` and `LEGACYDecimal128` are supported.
...
/**
 ...
 @param property The property whose minimum value is desired. Only properties of
                 types `int`, `float`, `double`, `LEGACYValue` and  `LEGACYDecimal128` are supported.
...
/**
 ...
 the write transaction.
     ...
     self.token = [person.dogs addNotificationBlock(LEGACYDictionary<NSString *, Dog *><LEGACYString, Dog> *dogs,
                                       LEGACYDictionaryChange *changes,
                                       NSError *error) {
 ...
 */
- (LEGACYNotificationToken *)addNotificationBlock:(void (^)(LEGACYDictionary<LEGACYKeyType, LEGACYObjectType> *_Nullable dictionary,
                                                         LEGACYDictionaryChange *_Nullable changes,
...
/**
 `-[LEGACYDictionary init]` is not available because `LEGACYDictionary`s cannot be created directly.
 `LEGACYDictionary` properties on `LEGACYObject`s are lazily created when accessed.
 */
- (instancetype)init __attribute__((unavailable("LEGACYDictionary cannot be created directly")));
/**
 `+[LEGACYDictionary new]` is not available because `LEGACYDictionary`s cannot be created directly.
 `LEGACYDictionary` properties on `LEGACYObject`s are lazily created when accessed.
 */
+ (instancetype)new __attribute__((unavailable("LEGACYDictionary cannot be created directly")));

@end

/**
 A `LEGACYDictionaryChange` object encapsulates information about changes to dictionaries
 that are reported by Realm notifications.

 `LEGACYDictionaryChange` is passed to the notification blocks registered with
 `-addNotificationBlock` on `LEGACYDictionary`, and reports what keys in the
 dictionary changed since the last time the notification block was called.
 */
@interface LEGACYDictionaryChange : NSObject
